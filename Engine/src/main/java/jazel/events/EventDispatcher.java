        ...
        for (Method method : listenerInstance.getClass().getMethods()) {
            if (method.isAnnotationPresent(EventHandler.class)) {
                if (method.getParameterCount() != 1) {
                    Log.getCoreLogger().error("Ignoring illegal event handler: {}: " +
                            "Wrong number of arguments (required: exactly 1)", method.getName());
                    continue;
                }
                ...
                if (!Event.class.isAssignableFrom(method.getParameterTypes()[0])) {
                    Log.getCoreLogger().error("Ignoring illegal event handler: {}: " +
                                    "Argument must extend {}", method.getName(),
                            Event.class.getName());
                    continue;
                }
            ...
            for (var listener : listenersForEvent) {
                if (!event.isHandled()) {
                    try {
                        ...
                        Log.getCoreLogger().error("Could not dispatch event {} to handler {}", event, listener.listenerMethod.getName());
                    }
                }
            }
        }
